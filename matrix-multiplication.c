...
struct Matrix {
    ...
    double* values;  /// 2D array containing all matrix values
};
...
void* calculate (void* param) {
    ...
    /// Work out result of multiplication by accessing row of Matrix A and column of Matrix B
    double result = 0;
    for (int i = 0; i < (*args).iterateCount; i++) {
        ...
        // Finally multiply and add to cumulative result
...
int saveToFile (char* inputFile, struct Matrix** matrices, int matricesCount) {
    ...
    fprintf(writeFile, "matrixresults2310336\n\n");
